...
def compute_delta_ae_from_verification(psp_data: Dict[str, Any], verification_results: List[Dict[str, Any]]) -> float:
    """
...
def aggregate_run_delta(pcap_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if phase_averages and phase_weights:
            ...
            if total_weight > 0:
                weighted_sum = sum(phase_averages.get(phase, 0.0) * phase_weights.get(phase, 0) 
                                for phase in set(phase_averages.keys()) | set(phase_weights.keys()))
                delta_run = weighted_sum / total_weight
            else:
                delta_run = sum(phase_averages.values()) / len(phase_averages) if phase_averages else 0.0
        else:
            delta_run = 0.0
