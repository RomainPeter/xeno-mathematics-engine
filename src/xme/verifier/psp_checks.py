...
def check_psp_acyclic(payload: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        return False, {
            "message": f"Error validating PSP: {str(e)}",
            "error": str(e)
        }
...
def check_psp_unique_ids(payload: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        return False, {
            "message": f"Error validating PSP IDs: {str(e)}",
            "error": str(e)
        }
...
def check_psp_topological_consistency(payload: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        if not nx.is_directed_acyclic_graph(G):
            return False, {
                "message": "Cannot check topological consistency: graph contains cycles"
            }
        ...
        try:
            ...
            return False, {
                "message": "Topological sorting failed",
                "n_blocks": len(psp.blocks)
            }
...
def check_psp_hasse_minimal_cover(payload: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        if not nx.is_directed_acyclic_graph(G):
            return False, {
                "message": "Cannot check Hasse cover: graph contains cycles"
            }
        ...
        return True, {
            "message": "Hasse cover is minimal",
            "n_edges": len(psp.edges)
        }
        
    except Exception as e:
        return False, {
            "message": f"Error checking Hasse cover: {str(e)}",
            "error": str(e)
        }
    ...
    try:
        ...
        for edge in psp.edges:
            if edge.src not in block_ids:
                edge_issues.append(f"Edge {edge.src}->{edge.dst}: source block '{edge.src}' not found")
            if edge.dst not in block_ids:
                edge_issues.append(f"Edge {edge.src}->{edge.dst}: destination block '{edge.dst}' not found")
...
def get_psp_obligations() -> List[Tuple[str, str, callable, str]]:
    ...
    return [
        ...
        ("psp_topo_consistency", "S0", check_psp_topological_consistency, "PSP topological order must be consistent"),
        ("psp_hasse_minimal", "S1", check_psp_hasse_minimal_cover, "PSP Hasse cover must be minimal"),
        ("psp_blocks_edges_consistency", "S1", check_psp_blocks_edges_consistency, "PSP blocks and edges must be consistent")
    ]
