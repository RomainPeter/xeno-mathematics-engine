...
def check_cegis_iterations_limit(payload: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        return False, {"message": f"Error checking CEGIS iterations: {str(e)}", "error": str(e)}
...
def check_cegis_secret_match(payload: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        return False, {"message": f"Error checking CEGIS secret match: {str(e)}", "error": str(e)}
...
def get_cegis_obligations() -> (
    ...
    return [
        (
            "cegis_iterations_limit",
            "S0",
            check_cegis_iterations_limit,
            "CEGIS iterations must be within limit",
        ),
        (
            "cegis_secret_match",
            "S0",
            check_cegis_secret_match,
            "CEGIS candidate must match expected secret",
        ),
