...
class Verifier:
    ...
    def run(self, obligation_ids: List[str], payload: Dict[str, Any], 
            level_filter: Optional[str] = None) -> VerificationReport:
        """
        ...
        report = VerificationReport(
            results=results,
            ok_all=all(result.ok for result in results)
        )
    ...
    def run_by_level(self, payload: Dict[str, Any], level: str) -> VerificationReport:
        ...
        """
        obligation_ids = [
            id for id, obligation in self.obligations.items()
            if obligation.level == level
        ]
