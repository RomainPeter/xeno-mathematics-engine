...
def check_ae_concepts_completeness(payload: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        all_attrs_intent_found = any(len(intent) == len(context.attributes) for (_, intent) in concepts)
...
def get_ae_obligations() -> List[Tuple[str, str, callable, str]]:
    ...
    return [
        ...
        ("ae_next_closure_stable", "S1", check_ae_next_closure_stable, "AE Next-Closure must be stable (idempotent)"),
