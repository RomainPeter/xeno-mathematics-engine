...
class ReportGenerator:
    ...
    def generate_markdown_report(self, metrics: Dict[str, Any]) -> str:
        ...
        if perf:
            ...
            md.append(f"- **Temps moyen:** {exec_analysis.get('average_execution_time_ms', 0):.2f}ms")
            ...
            md.append(f"- **Retries moyens par PCAP:** {efficiency.get('average_retries_per_pcap', 0):.2f}")
            md.append("")
        ...
        if quality:
            ...
            md.append(f"- **Violations d'obligations:** {obligation_analysis.get('obligation_violations', 0)}")
            md.append(f"- **Taux de conformité:** {obligation_analysis.get('compliance_rate', 0):.2%}")
            md.append("")
        ...
        if success_rate < 0.5:
            md.append("- ⚠️ **Taux de succès faible:** Considérer l'amélioration de la planification")
    ...
    def save_report(self, metrics: Dict[str, Any], output_dir: str, 
    ...
    def generate_comparison_report(self, metrics1: Dict[str, Any], 
                                 metrics2: Dict[str, Any]) -> str:
        """
        ...
        md.append(f"| Taux de succès | {success1:.2%} | {success2:.2%} | {success2 - success1:+.2%} |")
        ...
        if perf1 and perf2:
            ...
            md.append(f"**Temps d'exécution moyen:** {time1:.2f}ms → {time2:.2f}ms ({time2 - time1:+.2f}ms)")
