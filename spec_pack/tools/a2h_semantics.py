...
def find_anf() -> Tuple[str, Any]:
    candidates = ["spec_pack/anf.yaml", "spec_pack/anf.json", "spec_pack/ambition.json", "spec_pack/ambition.yaml"]
    for c in candidates:
        data = load_yaml_or_json(c)
        if data:
            return c, data
    raise FileNotFoundError("ANF not found (expected one of anf.yaml/anf.json/ambition.json/ambition.yaml under spec_pack/)")
...
def expand_required_from_rules(clauses: Dict[str,List[str]], rules: List[Dict[str,Any]]) -> Dict[str, List[str]]:
