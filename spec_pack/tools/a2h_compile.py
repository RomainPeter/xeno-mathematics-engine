...
def derive(anf):
    edges=[]
    def edge(src,dst,why): edges.append({"from":src,"to":dst,"rule":why})

    # Required core (doivent exister aujourd'hui)
    req_obl=set(); req_inv=set()
    req_tests={"S1":["H-S1-01"], "S2":["H-S2-01"]}
    opt_obl=set(); opt_inv=set()
    opt_tests={"S1":[], "S2":[], "S3":[]}
    ...
    for g in anf.get("guarantees",[]):
        ...
        if gid=="G-1":  # minimalité
            req_obl.add("O-1"); req_inv.add("I-6"); edge("G-1","O-1","minimality→obligation"); edge("G-1","I-6","minimality→invariant")
        if gid=="G-2":  # journal/explicabilité
            req_obl.add("O-2"); req_inv.add("I-7"); edge("G-2","O-2","journal→obligation"); edge("G-2","I-7","journal→invariant")
        if gid=="G-3":  # audit 24h (optionnel v0.2)
            opt_obl.add("O-3"); edge("G-3","O-3","audit24h→obligation")
        if gid=="G-4":  # borne K
            req_obl.add("O-4"); req_inv.add("I-5"); edge("G-4","O-4","trace_bound→obligation"); edge("G-4","I-5","trace_bound→invariant")
        if gid=="G-5":  # monotonie
            req_inv.add("I-1"); edge("G-5","I-1","monotonicity→invariant")

    # Constraints / Non-négociables / Menaces
    for c in anf.get("constraints",[]):
        cid=c.get("id")
        if cid=="C-3": req_obl.add("O-5"); req_inv.add("I-2"); edge("C-3","O-5","provenance→obligation"); edge("C-3","I-2","provenance→invariant")
        if cid=="C-4": opt_obl.add("O-6"); req_inv.add("I-3"); edge("C-4","O-6","determinism→obligation"); edge("C-4","I-3","determinism→invariant")
        if cid=="C-5": opt_obl.add("O-7"); edge("C-5","O-7","air_gapped→obligation")
    for nn in anf.get("non_negociables",[]):
        nid=nn.get("id")
        if nid=="NN-2": req_inv.add("I-4"); edge("NN-2","I-4","reversibility→invariant")
        if nid=="NN-3": opt_obl.add("O-12"); edge("NN-3","O-12","no_blackbox→obligation")
    for ta in anf.get("threat_actors",[]):
        tid=ta.get("id")
        if tid=="TA-1": opt_obl.add("O-8"); edge("TA-1","O-8","malicious_auditor→deny_with_proof")
        if tid=="TA-2": opt_obl.add("O-10"); edge("TA-2","O-10","data_poisoner→retro_implication")
...
def append_journal(manifest_hash, compiled_hash):
    ...
    with open(JRNL,"w",encoding="utf-8") as f:
        for o in rows: f.write(json.dumps(o,separators=( ",", ":" ))+"\n")
...
def main():
    ...
    write_json(OBL_FILE, compiled["required"] | {"optional_obligations": compiled["optional"]["obligations"]})
    write_json(INV_FILE, {"required": compiled["required"]["invariants"], "optional": compiled["optional"]["invariants"]})
    write_json(SHOCK_FILE, {"required": compiled["required"]["tests"], "optional": compiled["optional"]["tests"]})
...
"""
    ...
    with open(REPORT,"w",encoding="utf-8") as f: f.write(rep)
    ...
    if mode=="emit":
        ...
        print(json.dumps({"A2H_check_ok": ok, "missing_obligations": miss_obl, "missing_invariants": miss_inv, "tests_required_ok": tests_ok}, indent=2))
