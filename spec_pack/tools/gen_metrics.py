...
def run_s1():
    try:
        p=subprocess.run(["python","spec_pack/tools/verify.py","--s1"], capture_output=True, text=True, timeout=60)
        ok=(p.returncode==0)
...
def main():
    ...
    with open(MET,"w",encoding="utf-8") as f: json.dump(metrics,f,indent=2)
    ...
    with open(GRAPH,"w",encoding="utf-8") as f: json.dump(graph,f,indent=2)
