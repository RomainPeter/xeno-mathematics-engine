...
def v_trace_bound(decision, K):
    return {"pass": int(decision.get("trace_length", 999)) <= K, "details": {"trace_length": decision.get("trace_length")}}
...
def v_determinism(decision):  # smoke: same evidence_ids repeated
    ...
    for _ in range(3):  # quick check on samples
        if decision.get("evidence_ids", []) != eids: return {"pass": False, "details": "evidence_ids changed"}
...
def v_provenance(evidence_objects):
    for obj in evidence_objects:
        ...
        if not isinstance(h,str) or not HEX64.match(h): 
            return {"pass": False, "details": {"id": obj.get("id"), "issue":"bad_hash"}}
    return {"pass": True, "details": {}}
...
def main():
    ...
    ap.add_argument("--s1", action="store_true", help="Run S1 subset: minimality, trace bound, provenance presence")
    args = ap.parse_args()
    ...
    if args.s1:
        ...
        for d in decisions:
            ...
            results.append({"decision": d["id"], "minimality": r_min, "trace": r_trace, "determinism": r_det, "provenance": r_prov})
        ok = all(r["minimality"]["pass"] and r["trace"]["pass"] and r["determinism"]["pass"] and r["provenance"]["pass"] for r in results)
