...
def main():
    ...
    inject_ts = None; retro_ts = None
    for e in incidents:
        if e.get("type") == "contradiction_injection": inject_ts = parse_iso(e["timestamp"])
        if e.get("type") == "retro_rule_created": retro_ts = parse_iso(e["timestamp"])
    time_ok = False
