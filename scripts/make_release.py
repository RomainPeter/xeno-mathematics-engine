#!/usr/bin/env python3
import os, json, zipfile, subprocess, sys
...
def main():
    ...
    with zipfile.ZipFile(out_zip, "w", zipfile.ZIP_DEFLATED) as z:
        # Artifacts
        if Path("out/pcap").exists(): add_tree(z, Path("out/pcap"), "out/pcap")
        if Path("out/audit").exists(): add_tree(z, Path("out/audit"), "out/audit")
        if Path("out/metrics").exists(): add_tree(z, Path("out/metrics"), "out/metrics")
        # Docs
        if Path("docs/2pager.md").exists(): z.write("docs/2pager.md", "docs/2pager.md")
        if Path("LOGS.md").exists(): z.write("LOGS.md", "LOGS.md")
        if Path("README.md").exists(): z.write("README.md", "README.md")
