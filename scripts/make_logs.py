#!/usr/bin/env python3
import os, json, glob, datetime, hashlib, platform, subprocess
...
def main():
    ...
    if not att:
        digest = hashlib.sha256(json.dumps([e.get("operator") for e in pcap_entries], sort_keys=True).encode()).hexdigest()
        att = {"ts": datetime.datetime.utcnow().isoformat()+"Z", "pcap_count": len(pcap_entries), "digest": digest, "verdicts": []}
    ...
    lines.append(f"- Cache: {cache_stats['cache_files']} files (~{cache_stats['approx_size_kb']} KB)")
    lines.append("")
    ...
    lines.append("- Direction: hybrid orchestration (stochastic generation inside, deterministic verification outside).")
    lines.append("- Value: reduces supervision cost (PCAP journal, attestation, replayability), structures verification.")
    lines.append("- Question: do you see interest in large-scale autoformalization, and what minimum bar would you want for a micro-pilot?")
    lines.append("")

    LOGS_MD.write_text("\n".join(lines), encoding="utf-8")
    print(json.dumps({"ok": True, "pcap": len(pcap_entries), "digest": att.get("digest"), "logs": str(LOGS_MD)}, ensure_ascii=False))
