...
def main():
    ...
    if missing_files:
        ...
        if force_verify != "1":
            print(
                f"2cat verification skipped (missing files: {', '.join(missing_files)}). Set FORCE_VERIFY_2CAT=1 to enforce."
            )
            sys.exit(0)
        else:
            print(
                f"Missing files: {', '.join(missing_files)} and FORCE_VERIFY_2CAT=1 set. Failing."
            )
            sys.exit(2)
