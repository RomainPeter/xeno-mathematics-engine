...
def main():
    ...
    p_plan = PCAP(
        ...
        post={"plan": plan["plan"]},
        obligations=X.K,
        justification=VJustification(time_ms=plan["llm_meta"]["latency_ms"]),
        proof_state_hash=merkle_of({"H": X.H, "K": X.K}),
    ...
    for v in variants:
        ...
        delta = compute_delta(
            ...
            before_dir="demo_repo",
            after_dir=ws.work_dir,
        ...
        p_verify = PCAP(
            ...
            proof_state_hash=merkle_of(verdicts),
            toolchain={"python": "3.11"},
    ...
    if all_failed:
        p_rb = PCAP(
            ...
            case_id=case_id,
            pre={},
            post={"reason": "all variants failed"},
            obligations=X.K,
            justification=VJustification(time_ms=0, backtracks=1),
            proof_state_hash=merkle_of({"fail": True}),
            toolchain={"python": "3.11"},
            verdict="fail",
        )
        write_pcap(p_rb)
        ...
        p_replan = PCAP(
            ...
            case_id=case_id,
            pre={"K_prev": X.K[:-1]},
            post={"K_new": X.K, "plan": plan2["plan"]},
            obligations=X.K,
            justification=VJustification(time_ms=plan2["llm_meta"]["latency_ms"]),
            proof_state_hash=merkle_of({"K": X.K}),
            toolchain={"python": "3.11"},
            verdict="fail",
        )
        write_pcap(p_replan)
