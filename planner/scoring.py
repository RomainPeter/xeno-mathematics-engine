...
class UtilityCalculator:
    ...
    def estimate_psuccess(self, action: ActionVariant, context: Dict[str, Any]) -> float:
        ...
        # Probabilité finale
        final_probability = base_probability * historical_adjustment * context_adjustment * cost_adjustment
    ...
    def _get_historical_cost_multiplier(self, action_id: str) -> float:
        ...
        avg_cost = sum(r.get("cost", {}).get("audit_cost", 0) for r in action_records) / len(action_records)
    ...
    def record_action_result(self, action_id: str, verdict: str, cost: VJustification, context: Dict[str, Any]) -> None:
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        if successful_actions:
            avg_success_cost = sum(r.get("cost", {}).get("audit_cost", 0) for r in successful_actions) / len(successful_actions)
        ...
        if failed_actions:
            avg_failure_cost = sum(r.get("cost", {}).get("audit_cost", 0) for r in failed_actions) / len(failed_actions)
