...
class TestProofEngineOrchestrator:
    ...
    def test_create_initial_state(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert state.K == ["tests_ok", "lint_ok", "types_ok", "security_ok", "complexity_ok", "docstring_ok"]
    ...
    def test_execute_planning_phase_success(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            orchestrator = ProofEngineOrchestrator(temp_dir, "out")
            orchestrator.current_state = orchestrator._create_initial_state("test_case", "test goal")
            ...
            with patch.object(orchestrator.planner, 'propose_plan') as mock_plan:
                mock_plan.return_value = MagicMock(
                    ...
                    est_cost=2.0,
                    notes="Test plan",
                    llm_meta={"latency_ms": 1000, "model": "grok-2-fast"}
                )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(orchestrator.metrics_collector, 'collect_metrics') as mock_collect:
                ...
                with patch.object(orchestrator.report_generator, 'save_report') as mock_report:
                    mock_report.side_effect = ["out/metrics/report.md", "out/metrics/report.json"]
