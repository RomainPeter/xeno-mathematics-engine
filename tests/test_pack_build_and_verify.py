...
def test_pack_build_and_verify(tmp_path: Path):
    ...
    pcap_file.write_text('{"type": "action", "action": "test", "timestamp": "2024-01-01T00:00:00Z"}\n')
...
def test_cli_pack_build_and_verify(tmp_path: Path):
    ...
    # Test build
    r = subprocess.run([
        sys.executable, "-m", "xme.cli.main", "pack", "build",
        "--include", str(tmp_path / "*.json"),
        "--out", str(tmp_path)
    ...
    # Test verify
    r = subprocess.run([
        sys.executable, "-m", "xme.cli.main", "pack", "verify",
        "--pack", str(pack_path)
    ], capture_output=True, text=True)
