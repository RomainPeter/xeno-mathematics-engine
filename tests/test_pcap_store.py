...
def test_pcap_store_append():
    ...
    try:
        ...
        entry1 = PCAPEntry(action="action1", actor="actor1", timestamp=datetime.now(timezone.utc))
        entry2 = PCAPEntry(
            action="action2",
            actor="actor2",
...
def test_pcap_store_read_all():
    ...
    try:
        ...
        # Append some entries
        entry1 = PCAPEntry(
            action="read_test1", actor="test_actor", timestamp=datetime.now(timezone.utc)
        )
        entry2 = PCAPEntry(
            action="read_test2",
            actor="test_actor",
...
def test_pcap_store_directory_creation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        entry = PCAPEntry(
            action="deep_test", actor="test_actor", timestamp=datetime.now(timezone.utc)
        )
