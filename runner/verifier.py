...
class Verifier:
    ...
    def _replay_single_proof(self, proof: Proof, pcap: PCAP) -> Dict[str, Any]:
        ...
        try:
            ...
            if replayed_proofs:
                ...
                success = (
                    replayed_proof.passed == proof.passed and
                    replayed_proof.name == proof.name
                )
            else:
                success = False
    ...
    def _calculate_verdict(self, integrity_check: Dict[str, Any], 
                          replay_results: Dict[str, Any], 
                          cost_consistency: Dict[str, Any]) -> str:
        """Calcule le verdict final de vérification."""
        # Vérifier l'intégrité
        integrity_valid = (
            integrity_check.get("hash_valid", False) and
            integrity_check.get("timestamp_valid", False) and
            integrity_check.get("fields_valid", False)
        )
        ...
        # Vérifier la cohérence des coûts
        cost_valid = (
            cost_consistency.get("cost_consistency", False) and
            cost_consistency.get("time_consistency", False) and
            cost_consistency.get("risk_consistency", False)
        )
    ...
    def get_verification_stats(self) -> Dict[str, Any]:
        ...
        avg_verification_time = sum(
            r.get("verification_time_ms", 0) for r in self.verification_history
        ) / total_verifications
